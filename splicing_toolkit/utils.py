...
def gtf_to_bed12(path_gtf = None, out_dir="~/tmp"):
   """
   Converts the genes present in a GTF file to a BED12 for easy processing of transcriptomic information.

   Inputs:
   path_gtf: A path to the GTF file for generation of the bed12
      Input: str

   path_out: Output directory for the resulting gene, exon, and intron bed files.
      Input: str
   ...
   # List of genes
   #gene_list = gtf_genes['attributes'].str.extract(r'gene_id "(.*?)";', expand=False)
   ...
   coding_exons_info = coding_exons.groupby(by=['chrom', 'gene_id', 'strand']).agg({'exon_num': 'max',
                             'start':','.join,
                             'end':','.join,
   ...
   intron_df = coding_exons_info[coding_exons_info['exon_num'] > 1].reset_index()
   intron_bed12 = intron_coordinate_generator(intron_df['exon_num'], intron_df['gene_id'], intron_df['chrom'], intron_df['start'], intron_df['end'], intron_df['strand'])
   
   # Opening exon and general gene files in the outdir(out_path / 'GTF_genes.bed')
   exons_bed12.to_csv((out_path / 'GTF_exons.bed'), sep='\t', index=False, header=False)
   genes_bed12.to_csv((out_path / 'GTF_genes.bed'), sep='\t', index=False, header=False)
   intron_bed12.to_csv((out_path / 'GTF_introns.bed'), sep='\t', index=False, header=False)

   return 0

# returns a df containing in bed12 format intron coords
def intron_coordinate_generator(num_exons, gene_id, chroms, start_coords, end_coords, strand):
   """
   Tool for generating the intron dataframe, separated out in part because of the increased complexity required in 
   calculating the coordinates of the intron bed file from the GTF

   Inputs:
   All of the following inputs must be the same size and each row must correspond to the same feature row in the GTF
...
def parse_bed12_introns_exons(bed12_row):
   ...
   for i in range(block_count):
      ...
      exons.append([chrom, exon_start, exon_end, f"{name}_exon_{exon_number}", ".", strand])
      ...
      if i < block_count - 1:
         ...
         introns.append([chrom, intron_start, intron_end, f"{name}_intron_{intron_number}", ".", strand])
...
def splicing_classifier(LRS_url, output_save):
   ...
   df = data_gene_intersect.to_dataframe(header=None, names=["chrom", "start", "end", "name", "score", "strand", 'thickStart', 'thickEnd', 'rgb', 'blocks', 'blockLengths', 'blockStarts', 'geneChrom', 'geneStart', 'geneEnd', 'geneName', 'geneScore', 'geneStrand', 'geneThickStart', 'geneThickEnd', 'geneRgb', 'geneBlocks', 'geneBlockLengths', 'geneBlockStarts', 'overlapBases'])

   # Testing CASE REMOVE DURING PRODUCTION!!!!
   #df = df.loc[(df['geneName'] == 'SPAC12G12.15') | (df['geneName'] == 'SPAC1F5.10') | (df['geneName'] == 'SPAC1F5.11c')]

   # overlap_df drops most of the extra info about the genes to preserve geneName and basic geneinfo 
   overlap_df = df[['chrom', 'start', 'end', 'name', 'score', 'strand', 'thickStart', 'thickEnd', 'rgb', 'blocks', 'blockLengths', 'blockStarts', 'geneName', 'geneStart', 'geneEnd', 'geneBlocks', 'geneBlockStarts', 'geneBlockLengths', 'overlapBases']]

   # data_df contains information about the reads that map to specific genes, effectively a bed12 of those overlapping LRs
   data_df = df[['chrom', 'start', 'end', 'name', 'score', 'strand', 'thickStart', 'thickEnd', 'rgb', 'blocks', 'blockLengths', 'blockStarts']].drop_duplicates()
   ...
   for i in tqdm(data_df['name']):
      ...
      if name_df.shape[0] > 1:
         ...
         # TODO: Decide what metric determines a LR belongs to a gene, straight overlap or whichever gene has a 5' start site closest to the LR 5' start site?
      ...
      if introns_exist:
         ...
         # Should contain all exons and introns between those two points in the read
         ...
         # We want to define a tolerance (about how many bases are required for an overlap to officially occur?)
         # Current threshold is greater than 5 bases
         #lr_exons_gene_exons = long_read_exons_bed.intersect(expected_exon_bed, wo=True)
         ...
         # If there is a LR intron over a gene exon (exon skipping or noncanonical splice sites) OR if there is a LR exon over a gene intron but the number of introns over introns matches the expected intron count (internal intron splice site)
         if lr_introns_gene_exons_df.shape[0] > 0 or (lr_exons_gene_introns_df.shape[0] > 0 and (lr_introns_gene_introns_df.shape[0] == expected_introns_df.shape[0])):
